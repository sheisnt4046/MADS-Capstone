...
@st.cache
def recommend_flavor(flavor_matrix, km = km, X_ar = X_ar):
    # Find the wines with a similar cosine-sim value and order them from bigges number
    wine_data = km.transform(X_ar)
    distances = cdist(flavor_matrix.reshape(1, -1), wine_data, 'cosine')
...
AgGrid(sel_data, gridOptions = gridoptions2, columns_auto_size_mode='FIT_CONTENTS')
